...
const AskUserQuestions = forwardRef<HTMLDivElement, AskUserQuestionsProps>(
  function AskUserQuestions(
    ...
    useEffect(() => {
      ...
      queueMicrotask(() => setIsMac(/mac/i.test(platform)));
    }, []);
    ...
    useEffect(() => {
      ...
      if (!el) {
        queueMicrotask(() => setOtherMultiline(false));
        return;
      }
      const lh = parseFloat(getComputedStyle(el).lineHeight) || 18;
      queueMicrotask(() => setOtherMultiline(el.scrollHeight > lh * 1.5));
    }, [otherText]);
            ...
            >
              ...
              <AnimatePresence>
                {selectedGroups.map((group) => {
                  ...
                  return (
                    <motion.div
                      key={`selected-${group.start}-${group.end}`}
                      aria-hidden
                      className={cn(
