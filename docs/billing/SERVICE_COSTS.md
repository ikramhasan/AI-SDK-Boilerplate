...
| Supermemory | User memory search and memory ingestion via `searchMemories` and `addMemory` tools | Plans include a monthly dollar balance. The infrastructure rates shown on the pricing page are Memory at $0.005 per 1,000 SM tokens for plain text and $0.010 per 1,000 SM tokens for rich content, SuperRAG at $0.001 per 1,000 SM tokens for text mode and $0.002 for rich mode, Search and Traversal at $0.005 per 1,000 queries, and Operations at $0.10 per 1,000 operations. This app currently exposes search and plain-text memory tools. |
...
- Supermemory `searchMemories` is cheap per call, but `addMemory` scales with net-new SM tokens. The current tool response does not expose billable SM-token usage or deduplication status, so app billing uses a conservative plain-text estimate and marks that metadata as estimated.
